...
{
	public class EquipamentOrderMBWayPageCS : DefaultPage
	{
		...
		public async void createLayoutPhoneNumber()
		{
			...
			{
				Text = "Para confirmar a sua\n" + equipmentOrder.name + "\nefetue o pagamento de " + equipmentOrder.valor + "€.",
		...
		public EquipamentOrderMBWayPageCS(EquipmentOrder equipmentOrder)
		{
		...
		async Task<string> CreateMbWayPayment()
		{
			...
			string value_string = Convert.ToString(equipmentOrder.valor);
			string result = await paymentManager.CreateMbWayPayment(App.original_member.id, equipmentOrder.paymentid, equipmentOrder.paymentorderid, phoneValueEdit.entry.Text, value_string, App.member.email);
